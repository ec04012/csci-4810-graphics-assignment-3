...
# this is a modified version of the program used to compare time vs length of line
...
timings = []
draw_gridlines()
time_file = open('time_vs_length_line.csv', mode='w', newline="")
time_writer = csv.writer(time_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
time_writer.writerow(['Trial', 'x0', 'y1', 'x1', 'y0', 'length', 'time(s)'])
while running:
    check_for_exit()
    if num_drawn_lines < total_num_lines:
        while num_drawn_lines < total_num_lines:
            ...
            # write to csv
            time_writer.writerow([num_drawn_lines+1, x0, y0, x1, y1, length, duration])
